...
export const todaysSpecialService = {
    getTodaysSpecials: async (restaurantId: string): Promise<Meal[]> => {
        try {
            const specialsRef = collection(db, "restaurants", restaurantId, "todaysSpecial");
...
=            const meals = await Promise.all(
            ...
            return meals.filter((meal): meal is Meal => meal !== null) as Meal[];
        } catch (error) {
